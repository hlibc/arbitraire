...
static fxdpnt *arb_karatsuba_mul_core(fxdpnt *x, fxdpnt *y, fxdpnt *z, int base)
{
  if (y->len == 1) {
    ...
    //z->lp = z->len = x->len + 1;
	z->lp = z->len = x->len;
    mul2(x, y, &z, base, 10, 0);
    return z;
  } else if (x->len == 1) {
	mul2(y, x,  &z, base, 10, 0);
...
//	z->lp = z->len = y->len;
  ...
  fxdpnt *x1 = arb_expand(NULL, x->len - m + 1000);
  //x1 = arb_expand(x1, x->len - m);
  fxdpnt *y1 = arb_expand(NULL, y->len - m + 1000);
  //y1 = arb_expand(y1, y->len - m);
  fxdpnt *x0 = arb_expand(NULL, m  +1000);
  //x0 = arb_expand(x0, m);
  fxdpnt *y0 = arb_expand(NULL, m + 1000);
  //y0 = arb_expand(y0, m);
  ...
  fxdpnt *z1 = arb_expand(NULL, 1000);
  fxdpnt *z2 = arb_expand(NULL, 1000);
  fxdpnt *z3 = arb_expand(NULL, 1000);
  fxdpnt *z4 = arb_expand(NULL, 1000);
  fxdpnt *z5 = arb_expand(NULL,1000);
  ...
  fxdpnt *z6 = arb_expand(NULL, 1000);
  fxdpnt *z7 = arb_expand(NULL, 1000);
  ...
  fxdpnt *z8 = arb_expand(NULL, 1000);
